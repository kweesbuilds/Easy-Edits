...
- **DaVinci: "clip not found" / relink dialog on import** — The XML was generated by an older version that used `file:///` URIs instead of `file://localhost/`. Re-run `xml_builder.py` against the existing `cut_plan.json` to regenerate `edit.xml` with the correct URI format.
- **DaVinci: "timecode extents do not match any clip in media pool"** — Two possible causes: (1) Old XML generated before the timecode fix — re-run `xml_builder.py` on the existing `cut_plan.json` to regenerate with correct `<timecode>` elements. (2) Very old XML (pre-v1.5.4) where frame counts were computed with wrong fps_num — same fix applies.
