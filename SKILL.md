...
- **DaVinci: "timecode extents do not match any clip in media pool"** — The XML was generated by an older version that computed frame counts using `fps_timebase × fps_den` instead of the raw numerator from ffprobe. Re-run `xml_builder.py` against the existing `cut_plan.json` to regenerate with corrected frame values.
